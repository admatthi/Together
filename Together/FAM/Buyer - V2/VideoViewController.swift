    ...
    func queryforinfo() {
        ...
        for each in videoids {
            ...
            ref?.child("Influencers").child(selectedid).child("Plans").child(each).observeSingleEvent(of: .value, with: { (snapshot) in
                ...
                if var author2 = value?["DayNumber"] as? String {
...
//            selectedvideo = videolinks[videoids[indexPath.row]]!
            ...
            selecteddescription = videodescriptions[videoids[indexPath.row]]!
...
//            cell.titlelabel.sizeToFit()
            cell.timeago.text = videodaytitles[videoids[indexPath.row]]?.uppercased()
